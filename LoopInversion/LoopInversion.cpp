...
struct LoopInversionPass : public LoopPass {
  ...
  void FindLoopBoundAndCounter(Loop *L)
  {
    ...
    for (Instruction &Instr : *LoopBasicBlocks[0]) {
      if (isa<ICmpInst>(&Instr)) {
        ...
        // Ako se granica cuva u promenljivoj, onda: VariablesMap[Instr.getOperand(1)];
  ...
  void LoopInversion(Loop *L)
  {
    ...
    // ubacuje se ispred prvog BB koji se odnosi na petlju
    NewBasicBlock->insertInto(LoopBasicBlocks.front()->getParent(),
...
//    br i1 %5, label %6, label %10
    ...
    Value *CMP = Builder.CreateICmp(CmpInst::ICMP_SLT, Load,
                                    ConstantInt::get(Type::getInt32Ty(NewBasicBlock->getContext()), BoundValue));
